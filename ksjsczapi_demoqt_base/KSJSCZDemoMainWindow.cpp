...
, m_nLastShowTick(-1)
{
	...
	connect(ui->ALedCheckBox, SIGNAL(stateChanged(int)), this, SLOT(OnALedCheckBoxChanged(int)));
	connect(ui->BLedCheckBox, SIGNAL(stateChanged(int)), this, SLOT(OnBLedCheckBoxChanged(int)));
	connect(ui->IOOUT1CheckBox, SIGNAL(stateChanged(int)), this, SLOT(OnIO1CheckBoxChanged(int)));
	connect(ui->IOOUT2CheckBox, SIGNAL(stateChanged(int)), this, SLOT(OnIO2CheckBoxChanged(int)));
	connect(ui->IOOUT3CheckBox, SIGNAL(stateChanged(int)), this, SLOT(OnIO3CheckBoxChanged(int)));
...
void CKSJSCZDemoMainWindow::UpdateUiSetting()
{
	...
	ui->StaticText_TriggerDelay->setText(QString::number(ulValueMin) + "-" + QString::number(ulValueMax));
	...
	//KSJSCZ_GetMirror(0, &m_bIsMirror);
	//ui->MirrorCheckBox->setCheckState(m_bIsMirror ? Qt::Checked : Qt::Unchecked);

	//KSJSCZ_GetMirror(0, &m_bIsFlip);
	//ui->FlipCheckBox->setCheckState(m_bIsFlip ? Qt::Checked : Qt::Unchecked);

	unsigned long  ulRegValue;
	unsigned short usRegAddress = 0x70;

	KSJSCZ_RdRegFPGA(0, usRegAddress, &ulRegValue);

	printf("\r\n Reg Addr: 0x%04X  --  Val: 0x%08X \r\n ", usRegAddress, ulRegValue);

	ui->LineEdit_RegAddress->setText(QString("0x%1").arg(usRegAddress, 4, 16, QChar('0')));
	ui->LineEdit_RegValue->setText(QString("0x%1").arg(ulRegValue, 8, 16, QChar('0')));

	bool bGpioValue = false;

	KSJSCZ_GpioGet(0, &bGpioValue);
	ui->IOIN1CheckBox->setCheckState(bGpioValue ? Qt::Checked : Qt::Unchecked);

	KSJSCZ_GpioGet(1, &bGpioValue);
	ui->IOIN2CheckBox->setCheckState(bGpioValue ? Qt::Checked : Qt::Unchecked);

	KSJSCZ_GpioGet(2, &bGpioValue);
	ui->IOIN3CheckBox->setCheckState(bGpioValue ? Qt::Checked : Qt::Unchecked);

	KSJSCZ_GpioGet(3, &bGpioValue);
	ui->IOOUT1CheckBox->setCheckState(bGpioValue ? Qt::Checked : Qt::Unchecked);

	KSJSCZ_GpioGet(4, &bGpioValue);
	ui->IOOUT2CheckBox->setCheckState(bGpioValue ? Qt::Checked : Qt::Unchecked);

	KSJSCZ_GpioGet(5, &bGpioValue);
	ui->IOOUT3CheckBox->setCheckState(bGpioValue ? Qt::Checked : Qt::Unchecked);

	ui->ALedCheckBox->setCheckState(m_bALedShine ? Qt::Checked : Qt::Unchecked);
	ui->BLedCheckBox->setCheckState(m_bBLedShine ? Qt::Checked : Qt::Unchecked);
...
void CKSJSCZDemoMainWindow::OnReadReg()
{
	...
	{
		QString strTemp = strRegAddress.right(strRegAddress.length() - 2);

		usRegAddress = strTemp.toInt(0, 16);
	}
	else
	{
		usRegAddress = strRegAddress.toInt();
	}

	usRegAddress = usRegAddress / 4 * 4;
	//if (ulRegValue.left(2).toUpper() == "0x")
	//{
	//	ulRegValue = strRegValue.right(strRegValue.length() - 2).toInt(0, 16);
	//}
	//else
	//{
	//	ulRegValue = strRegValue.toInt();
	//}
...
void CKSJSCZDemoMainWindow::OnWriteReg()
{
	...
	if (strRegAddress.left(2).toUpper() == "0X")
	{
		usRegAddress = strRegAddress.right(strRegAddress.length() - 2).toInt(0, 16);
	}
	else
	{
		usRegAddress = strRegAddress.toInt();
	...
	if (strRegValue.left(2).toUpper() == "0X")
	{
		ulRegValue = strRegValue.right(strRegValue.length() - 2).toInt(0, 16);
	}
	else
...
void CKSJSCZDemoMainWindow::OnALedCheckBoxChanged(int value)
{
	m_bALedShine = (value == Qt::Checked);
...
void CKSJSCZDemoMainWindow::OnBLedCheckBoxChanged(int value)
{
	m_bBLedShine = (value == Qt::Checked);
...
void CKSJSCZDemoMainWindow::OnIO1CheckBoxChanged(int value)
{
	KSJSCZ_GpioSet(3, value == Qt::Checked);
}

void CKSJSCZDemoMainWindow::OnIO2CheckBoxChanged(int value)
{
	KSJSCZ_GpioSet(4, value == Qt::Checked);
}

void CKSJSCZDemoMainWindow::OnIO3CheckBoxChanged(int value)
{
	KSJSCZ_GpioSet(5, value == Qt::Checked);
}
